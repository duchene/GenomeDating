...
process.sim.chronog.2 <- function(chron, pms.exp.rate = c(500, 1000), monoph = list(paste0("t", 1:20), paste0("t", 1:10)), topos = 5, sprsteps = 5, seed = 1234, slen = 1000, reps = 20){
		      
	phylogs <- list()	
	als <- list()
	outgroup1 <- chron$tip.label[Descendants(chron, Children(chron, (length(chron$tip.label) + 1))[2], type = "tips")[[1]]]
	
	for(i in 1:length(pms.exp.rate)){
	      phylogs[[length(phylogs) + 1]] <- chron
	      phylogs[[length(phylogs)]]$edge.length <- chron$edge.length * rexp(length(phylogs[[length(phylogs)]]$edge.length), rate = pms.exp.rate[i])
