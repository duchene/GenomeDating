# Process a pair of simulated chronograms. Take pair of chronogs in a list. Simulate data from them. Re-estimate phylograms. Output gene tree list. NOTE: THESE PARAMETERS FOR SIMULATION DID NOT RESULT IN TWO CLUSTERS USING MDS, SUCH THAT NEW PARAMETERS ARE NEEDED. SEE THE SECOND VERSION OF THIS SCRIPT.
...
process.gd.sim <- function(treepair){
	       ...
	       for(i in 1:100){
			tr1pm1[[i]] <- treepair[[1]]
			tr1pm1[[i]]$edge.length <- rlnorm(length($edge.length), meanlog = log(0.005), sdlog = 0.05)
			tr2pm1[[i]] <- treepair[[2]]
			tr2pm1[[i]]$edge.length <- rlnorm(length($edge.length), meanlog = log(0.005), sdlog = 0.05)
			tr1pm2[[i]] <- treepair[[1]]
			tr1pm2[[i]]$edge.length <- rlnorm(length($edge.length), meanlog = log(0.01), sdlog = 0.05)
			tr2pm2[[i]] <- treepair[[2]]
			tr2pm2[[i]]$edge.length <- rlnorm(length($edge.length), meanlog = log(0.01), sdlog = 0.05)
			tr1pm3[[i]] <- treepair[[1]]
			tr1pm3[[i]]$edge.length <- rlnorm(length($edge.length), meanlog = log(0.015), sdlog = 0.05)
			tr2pm3[[i]] <- treepair[[2]]
			tr2pm3[[i]]$edge.length <- rlnorm(length($edge.length), meanlog = log(0.015), sdlog = 0.01)
	       }
